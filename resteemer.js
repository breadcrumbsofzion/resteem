...
const CONFIG_FILE = `${__filename.slice(__dirname.length + 1, -3)}.json`;
...
const post_and_upvote_comment = (parent_author, parent_permlink, body) => {
  ...
  steem.broadcast.comment(
    ...
    '', // Title
    body, 
    { tags: ['schoolofsteem'], app: 'schoolofsteem' }, // Json Metadata
    function(err, result) {
      if (result && !err) {
        log("commented!");
        voting_post(config.account, permlink, 420);
      } else {
        log(err);
      } 
    }
